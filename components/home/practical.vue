...
<template>
  ...
  <view class="free-list">
    <view class="new-course">
      <view class="new-course-list">
        <view class="new-course-item" v-for="item in courseList " :key="item.id">
          <view class="new-course-item-image" style="border-radius: 8rpx;">
            <image :src="item.courseCover" mode="aspectFill"></image>
          </view>
          <view class="new-course-item-info">
            <view class="new-course-item-title">
              {{ item.courseName }}
            </view>
            <view class="new-course-item-type">
              {{ getCourseType(item.courseLevel) }}
            </view>
            <view class="new-course-item-price-info">
              <view class="new-course-item-price" v-if="item.discountPrice===0">
                <view>免费</view>
              </view>
              <view class="new-course-item-price" v-else>
                <view>限时优惠</view>
                <view>¥{{ item.discountPrice }}</view>
              </view>
              <view class="new-course-item-studied">
...
.new-course-item-image {
  width: 237rpx;
  height: 186rpx;
  border-radius: 8rpx;
}

.new-course-item-info {
  display: flex;
...
.new-course-item-type {
  font-size: 24rpx;
  font-weight: 400;
  color: #c5c5c5;
}

.new-course-item-price-info {
  display: flex;
  justify-content: space-between;
  //align-items: center;
}

.new-course-item-price {
  display: flex;
  justify-content: space-between;
  font-size: 21rpx;
  font-weight: 600;
...
.new-course-item-price view:nth-child(2) {
  padding-left: 21rpx;
}

.new-course-item-studied {
  font-size: 21rpx;
  font-weight: 400;
  color: #C5C5C5;
}

.new-course-item-image image {
  width: 100%;
  height: 100%;
  border-radius: 8rpx;
}
